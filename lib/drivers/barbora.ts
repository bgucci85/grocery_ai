...
export async function ensureLoggedIn(page: Page, log: LogSink): Promise<void> {
  ...
  const cloudflareSelectors = [
    ...
    'text=barbora.lt needs to review the security',
    '#challenge-form',
  ...
  if (hasCloudflare) {
    log.warn("⚠️  [barbora] Cloudflare challenge detected!");
    log.warn("📋 [barbora] Please solve the CAPTCHA manually in the browser window");
    log.warn("⏳ [barbora] Waiting up to 60 seconds for you to complete the challenge...");
    ...
    try {
      ...
      log.info("✅ [barbora] Cloudflare challenge solved! Continuing...");
      await page.waitForTimeout(2000); // Let page fully load
    } catch (error) {
      log.error("❌ [barbora] Timeout waiting for Cloudflare challenge to be solved");
