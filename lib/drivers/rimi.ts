...
export async function ensureLoggedIn(page: Page, log: LogSink): Promise<void> {
  ...
  if (needsLogin) {
    log.warn("[rimi] Please log in manually in the browser window. Session will be saved.");
    log.warn("[rimi] Waiting 90 seconds for manual login...");
    await page.waitForTimeout(90000);
  } else {
    log.info("[rimi] Already logged in (session restored)");
  }
}
...
export async function addByUrl(
  ...
  try {
    ...
    // Strategy 1: Look for specific Rimi button classes/text
    const rimiSelectors = [
    ...
    for (const selector of rimiSelectors) {
      try {
        const btn = page.locator(selector).first();
        if (await btn.count() > 0 && await btn.isVisible({ timeout: 2000 })) {
          await btn.click();
          added = true;
          log.info(`[rimi] Found button with selector: ${selector}`);
          break;
        }
      } catch {
        continue;
...
export async function addByQuery(
  ...
  try {
    ...
    for (const selector of searchSelectors) {
      try {
        ...
        if (await input.count() > 0) {
          ...
          if (visible) {
            searchInput = input;
            log.info(`[rimi] Found search input with selector: ${selector}`);
            break;
          }
        }
      } catch {
    ...
    if (!searchInput) {
      ...
      log.warn(`[rimi] Found ${inputs.length} input elements on page`);
    ...
    if (options.useOpenAI) {
      log.info(`[rimi] Parsing query with AI...`);
      parsedQuery = await parseGroceryQuery(query);
      log.info(`[rimi] Parsed: product="${parsedQuery.productName}", qty="${parsedQuery.desiredQuantity || 'any'}"`);
    ...
    for (let i = 0; i < searchVariations.length && products.length === 0; i++) {
      ...
      log.info(`[rimi] Trying search: "${term}"`);
      ...
      if (products.length > 0) {
        log.info(`[rimi] Found ${products.length} products with "${term}"`);
        break;
      } else if (i < searchVariations.length - 1) {
        log.warn(`[rimi] No products with "${term}", trying next variation...`);
    ...
    if (options.useOpenAI) {
      log.info(`[rimi] 🤖 AI AGENT ENABLED - Using GPT-4o for intelligent selection...`);
      ...
      log.warn(`[rimi] ⚠️  AI AGENT DISABLED - Using basic string matching (enable "Use OpenAI assist" for better results)`);
      let bestScore = 0;
      ...
      log.info(`[rimi] Selected by string similarity: ${products[bestIndex].title}`);
    }
    ...
    log.info(`[rimi] ✓ Clicked add for "${selected.title}"`);
    ...
    for (const selector of confirmSelectors) {
      try {
        ...
        if (await btn.isVisible({ timeout: 1000 })) {
          await btn.click();
          log.info(`[rimi] ✅ Confirmed addition to cart`);
          confirmed = true;
          break;
        }
      } catch {
    ...
    if (!confirmed) {
      log.info(`[rimi] No confirmation button found (item might already be in cart)`);
    }
